...
def test_archive_scope_lexicon_search_work_and_timeline(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert "Archive Planning 已完成" in created.text
        ...
        client.post(
            "/archives/1/concept-sets",
            data={
                "name": "Chemical Space",
                "description": "Anchor concept",
        ...
        lexicon = client.post(
            ...
            data={
                "name": "Representation",
                "description": "Representation methods",
    ...
    async def verify():
        async with app.state.database.get_session() as session:
            ...
            assert len(hidden) == 2
            assert all(
                item.subscription_type == "archive_search" and not item.enabled for item in hidden
            )
            assert (
                await session.execute(select(func.count(ArchiveRevision.id)))
            ).scalar_one() == 12
